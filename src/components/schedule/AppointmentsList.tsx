        ...
        <CardTitle className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 md:gap-4">
          <div className="flex items-center space-x-2 min-w-0">
            ...
            <span className="text-xs md:text-sm lg:text-base truncate">
              {t('appointments_for')} {selectedDate ? getDateLabel(quickDateOptions.find(opt => opt.value === selectedDate)?.days || 0) : 'Selected Date'}
            </span>
          </div>
          <div className="flex items-center space-x-1 md:space-x-2 flex-shrink-0">
            ...
            >
              ...
              <span className="hidden sm:inline text-xs md:text-sm">{t('print_schedule')}</span>
            </Button>
          </div>
        </CardTitle>
      </CardHeader>
      <CardContent>
        {filteredAppointments.length === 0 ? (
          <div className="text-center py-6 md:py-8">
            ...
            <p className="text-gray-500 text-sm md:text-base">{t('no_appointments_found')}</p>
          </div>
        ) : (
